...
{#- AE#153 (Entrega 2 da #147/#148, ADR 0014): `insumo_escopo`/`insumo_recorte` carimbam,
    ...
    mesmo `insumos_medidos` (o array por premissa) ficando só nos ramos do 1X2 e, desde a
    AE#202, do Handicap. -#}
...
cand_1x2 AS (
    SELECT
        ...
        -- AE#153: um dos dois ramos com o array de verdade (o outro é o Handicap, AE#202)
        -- — os demais publicam [] (array vazio; NULL não sobrevive à escrita no BigQuery,
        -- ver futebol_insumos_medidos_vazio()), porque o modelo deles ainda não tem
        -- a coluna.
        p.insumos_medidos,
...
cand_btts AS (
    SELECT
        ...
        -- AE#153: array só no 1X2 nesta entrega — o modelo de Ambos Marcam ainda não tem a coluna.
        -- ⚠️ [] explícito, não CAST(NULL ...): array NULL não sobrevive à escrita no
        -- BigQuery (volta como [] — ver ⚠️ em macros/premissas_valores_medidos.sql).
        {{ futebol_insumos_medidos_vazio() }} AS insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,

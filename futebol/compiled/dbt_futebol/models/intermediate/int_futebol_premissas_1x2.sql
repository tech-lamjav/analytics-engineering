...
cegueira AS (
    SELECT
        ...
        -- AE#153 (Entrega 2 da #147/#148, ADR 0014): o valor que cada insumo tinha, não só
        -- se a premissa acendeu. Gerado do mesmo catálogo, no mesmo lugar — as colunas de
        -- CTE ainda existem como identificador bruto aqui.
        ARRAY(
        SELECT AS STRUCT premissa, insumo, valor
        FROM UNNEST([
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'forca_mismatch', CAST(NULL AS STRING)) AS premissa,
                's_gf_venue' AS insumo,
                CAST(s_gf_venue AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'forca_mismatch', CAST(NULL AS STRING)) AS premissa,
                'o_ga_venue' AS insumo,
                CAST(o_ga_venue AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'superioridade_xg', CAST(NULL AS STRING)) AS premissa,
                's_xg_for' AS insumo,
                CAST(s_xg_for AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'superioridade_xg', CAST(NULL AS STRING)) AS premissa,
                'o_xg_against' AS insumo,
                CAST(o_xg_against AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw') AND (s_is_home), FALSE), 'mando', CAST(NULL AS STRING)) AS premissa,
                'pct_pts_home' AS insumo,
                CAST(pct_pts_home AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw') AND (NOT s_is_home), FALSE), 'mando', CAST(NULL AS STRING)) AS premissa,
                'aprov_fora' AS insumo,
                CAST(aprov_fora AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'desfalque_adversario', CAST(NULL AS STRING)) AS premissa,
                's_missing' AS insumo,
                CAST(s_missing AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'desfalque_adversario', CAST(NULL AS STRING)) AS premissa,
                'o_missing' AS insumo,
                CAST(o_missing AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'superioridade_tabela', CAST(NULL AS STRING)) AS premissa,
                's_rank' AS insumo,
                CAST(s_rank AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'superioridade_tabela', CAST(NULL AS STRING)) AS premissa,
                'o_rank' AS insumo,
                CAST(o_rank AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'superioridade_tabela', CAST(NULL AS STRING)) AS premissa,
                's_ppg' AS insumo,
                CAST(s_ppg AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'superioridade_tabela', CAST(NULL AS STRING)) AS premissa,
                'o_ppg' AS insumo,
                CAST(o_ppg AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'forma', CAST(NULL AS STRING)) AS premissa,
                'n_wins_last5' AS insumo,
                CAST(n_wins_last5 AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'h2h_favoravel', CAST(NULL AS STRING)) AS premissa,
                'h2h_total' AS insumo,
                CAST(h2h_total AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'h2h_favoravel', CAST(NULL AS STRING)) AS premissa,
                's_wins' AS insumo,
                CAST(s_wins AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((outcome <> 'Draw'), FALSE), 'desfalque_proprio', CAST(NULL AS STRING)) AS premissa,
                's_missing' AS insumo,
                CAST(s_missing AS FLOAT64) AS valor
            )
        ])
...
SELECT
    ...
    -- AE#153: valor medido por (premissa, insumo). Ver docstring de futebol_insumos_medidos.
    insumos_medidos,

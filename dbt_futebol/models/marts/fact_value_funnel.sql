...
-- ============================================================================
{#- AE#153 (Entrega 2 da #147/#148, ADR 0014): `insumo_escopo`/`insumo_recorte` carimbam,
    POR LINHA, o eixo que estava em vigor QUANDO ESTA LINHA FOI GRAVADA — não uma leitura
    do catálogo (que descreve o eixo em geral, sempre hoje). São escalares do build
    inteiro (a mesma var vale pra toda linha), então entram nos CINCO ramos de uma vez,
    mesmo `insumos_medidos` (o array por premissa) ficando só nos ramos dos mercados de
    `futebol_mercados_com_insumos_medidos()` — 1X2 (AE#153), Handicap (AE#202), Ambos marcam
    (AE#208) e Dupla chance (AE#209). -#}
...
-- aqui.
denominador AS (
    SELECT market, lado, teto FROM {{ ref('futebol_teto_nota_contexto') }}
),
...
cand_1x2 AS (
    SELECT
        ...
        -- AE#153: o primeiro ramo com o array de verdade (depois vieram o Handicap, AE#202,
        -- o Ambos marcam, AE#208, e a Dupla chance, AE#209) — os demais publicam [] (array vazio; NULL não
        -- sobrevive à escrita no BigQuery, ver futebol_insumos_medidos_vazio()), porque o
        -- modelo deles ainda não tem a coluna.
        p.insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,
        '{{ eixos.recorte }}'                           AS insumo_recorte,
...
cand_ou AS (
    SELECT
        ...
        -- AE#153: array só no 1X2 nesta entrega — o modelo de Gols ainda não tem a coluna.
        -- ⚠️ [] explícito, não CAST(NULL ...): array NULL não sobrevive à escrita no
        -- BigQuery (volta como [] — ver ⚠️ em macros/premissas_valores_medidos.sql).
        {{ futebol_insumos_medidos_vazio() }} AS insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,
        '{{ eixos.recorte }}'                           AS insumo_recorte,
...
cand_ah AS (
    SELECT
        ...
        -- AE#202: o Handicap é o segundo ramo com o array de verdade (depois do 1X2, AE#153).
        -- Linha de jogo já apitado antes deste deploy fica com [] para sempre — append-only
        -- (ADR 0011), mesmo custo que o 1X2 aceitou.
        p.insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,
        '{{ eixos.recorte }}'                           AS insumo_recorte,
...
cand_btts AS (
    SELECT
        ...
        -- AE#208: o terceiro ramo com o array de verdade (depois do 1X2, AE#153, e do
        -- Handicap, AE#202; a Dupla chance veio depois, AE#209). Linha de jogo já apitado antes deste deploy fica com [] para
        -- sempre — append-only (ADR 0011), mesmo custo que os dois anteriores aceitaram.
        p.insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,
        '{{ eixos.recorte }}'                           AS insumo_recorte,
...
cand_dc AS (
    SELECT
        ...
        -- AE#209: o quarto ramo com o array de verdade. A "12" não tem linha no modelo de
        -- premissas, o LEFT JOIN traz NULL e a escrita o guarda como [] (NULL não sobrevive
        -- em coluna ARRAY) — a mesma ausência do score dela. Linha de jogo já apitado antes
        -- deste deploy fica com [] para sempre (append-only, ADR 0011).
        p.insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,
        '{{ eixos.recorte }}'                           AS insumo_recorte,

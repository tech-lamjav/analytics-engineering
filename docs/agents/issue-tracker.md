...
Issues and PRDs for this repo live as GitHub issues in `tech-lamjav/analytics-engineering`. Use the `gh` CLI for all operations.
...
auto-close recognizes. `Closes AE#153` and `[AE#153](url)` do **not** close the issue on
merge (happened here on PR #154, and again on data-engineering's PR #99). Cross-repo:
`Closes owner/repo#N` (full path). A readable `[AE#153](url)` link elsewhere in the body is

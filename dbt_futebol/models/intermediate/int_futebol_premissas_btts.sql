...
cegueira AS (
    SELECT
        ...
        -- AE#208 (filha da #147, mesmo gerador do 1X2 e do Handicap, ADR 0014): o valor que
        -- cada insumo tinha, não só se a premissa acendeu. A aplicabilidade é a saída, como
        -- na cegueira: Yes publica as 4 do Sim, No as 3 do Não, e nenhuma linha sai vazia.
        -- Os insumos são do mandante e do visitante (home_*/away_*), não S/O — no BTTS
        -- nenhum dos dois times é o lado apostado.
        {{ futebol_insumos_medidos('int_futebol_premissas_btts') }} AS insumos_medidos
    FROM scored s
)
...
SELECT
    ...
    -- o valor medido de cada premissa aplicável (AE#208). ARRAY: o sync o pula, e quem
    -- atravessa para o Postgres é o fact_insumos_medidos.
    insumos_medidos,

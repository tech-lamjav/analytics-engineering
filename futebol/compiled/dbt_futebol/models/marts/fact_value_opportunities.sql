...
WHERE janela_e_corrente
  ...
  AND NOT COALESCE(
        (
        _fx_status_short IN ('FT', 'AET', 'PEN', 'CANC', 'ABD', 'AWD', 'WO', '1H', 'HT', '2H', 'ET', 'BT', 'P', 'LIVE')
        OR (
            TIMESTAMP_ADD(_fx_kickoff_utc, INTERVAL 24 HOUR)
                < CURRENT_TIMESTAMP()
            -- COALESCE, e não `_fx_status_short NOT IN (...)` direto: status nulo tem de
            -- ser alcançado pela carência, não escapar dela por NULL. Ver o cabeçalho.
            AND COALESCE(_fx_status_short, '') NOT IN ('PST', 'SUSP', 'INT')
        )
    ),
        FALSE
      )
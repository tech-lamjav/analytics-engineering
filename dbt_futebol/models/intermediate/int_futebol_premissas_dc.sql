...
cegueira AS (
    SELECT
        ...
        -- AE#209 (filha da #147, mesmo gerador dos outros mercados, ADR 0014): o valor que cada
        -- insumo tinha. As 4 premissas se aplicam às duas saídas, então 1X e X2 publicam 10
        -- entradas cada e nenhuma sai vazia. Os três x_* são o VEREDITO de premissas do 1X2
        -- do lado coberto, e saem 1.0/0.0 (NULL quando cegos lá) — declarados em `booleanos`
        -- no catálogo. O número por trás do veredito não é copiado: está no mesmo fact, na
        -- linha do 1X2 do mesmo jogo com outcome = s_1x2_outcome (Home para 1X, Away para X2).
        {{ futebol_insumos_medidos('int_futebol_premissas_dc') }} AS insumos_medidos
    FROM scored s
)
...
SELECT
    ...
    -- o valor medido de cada premissa (AE#209). ARRAY: o sync o pula, e quem atravessa para o
    -- Postgres é o fact_insumos_medidos.
    insumos_medidos,

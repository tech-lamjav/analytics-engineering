...
board AS (
    SELECT
        ...
        b.score,
        ROUND(b.edge, 10) AS edge,
        ROUND(b.best_odd, 10) AS best_odd,
        ROUND(b.avg_odd, 10) AS avg_odd,
        ROUND(b.prob_justa_fechamento, 10) AS prob_justa_fechamento,
        b.pts_valor,
        b.pts_premissas,
        b.premissas_sem_dado,
        b.pts_corroboracao,
        b.penalidades,
        b.penalidades_globais_pts,
        b.penalidades_especificas_pts,
        b.pen_odd_outlier,
        b.pen_poucas_casas,
        b.pen_odd_longshot,
        b.pen_odd_juice,
        b.modelo_api_concorda,
        b.linha_sharp_confirma,
        b.best_book,
        b.n_casas,
        b.valor_fonte,
        b.pin_n_outcomes,
        ...
        b.competition,
        b.season
    FROM `smartbetting-dados`.`futebol`.`fact_value_opportunities` b
...
funil_publicavel AS (
    SELECT
        ...
        f.janela,
        f.score,
        ROUND(f.edge, 10) AS edge,
        ROUND(f.best_odd, 10) AS best_odd,
        ROUND(f.avg_odd, 10) AS avg_odd,
        ROUND(f.prob_justa_fechamento, 10) AS prob_justa_fechamento,
        f.pts_valor,
        f.pts_premissas,
    ...
    WHERE f.janela_e_corrente
      ...
      AND COALESCE(fx._fx_kickoff_utc > CURRENT_TIMESTAMP(), TRUE)
      AND NOT COALESCE(
            (

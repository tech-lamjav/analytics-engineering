...
SELECT
    ...
    rank_description,
    CASE
        WHEN rank_description IS NULL THEN NULL
        WHEN rank_description LIKE '%Relegation%' THEN 'rebaixamento'
        WHEN rank_description LIKE '%Champions League%'
          OR rank_description LIKE '%Europa League%'
          OR rank_description LIKE '%Conference League%'
          OR rank_description LIKE '%Libertadores%'
          OR rank_description LIKE '%Sudamericana%'
          OR rank_description LIKE '%UEFA%'
          OR rank_description LIKE '%ECL%'
        THEN 'vaga_continental'
        WHEN rank_description LIKE '%Promotion%' THEN 'promocao'
        ELSE 'classificacao'
    END AS categoria,
    COUNT(*)   AS linhas,

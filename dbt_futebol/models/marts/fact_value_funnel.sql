...
{#- AE#153 (Entrega 2 da #147/#148, ADR 0014): `insumo_escopo`/`insumo_recorte` carimbam,
    ...
    `futebol_mercados_com_insumos_medidos()` — 1X2 (AE#153), Handicap (AE#202) e Ambos marcam
    (AE#208). -#}
...
cand_1x2 AS (
    SELECT
        ...
        -- e o Ambos marcam, AE#208) — os demais publicam [] (array vazio; NULL não
...
cand_btts AS (
    SELECT
        ...
        -- Handicap, AE#202). Linha de jogo já apitado antes deste deploy fica com [] para
...
cand_dc AS (
    SELECT
        ...
        -- AE#153: array só no 1X2 nesta entrega — o modelo de Dupla Chance ainda não tem a coluna.
        -- ⚠️ [] explícito, não CAST(NULL ...): array NULL não sobrevive à escrita no
        -- BigQuery (volta como [] — ver ⚠️ em macros/premissas_valores_medidos.sql).
        {{ futebol_insumos_medidos_vazio() }} AS insumos_medidos,
        '{{ eixos.escopo }}'                            AS insumo_escopo,

...
cegueira AS (
    SELECT
        ...
        FROM UNNEST([
            STRUCT(
                IF(COALESCE((is_favorito), FALSE), 'supremacia', CAST(NULL AS STRING)) AS premissa,
                's_rank' AS insumo,
            ...
            STRUCT(
                IF(COALESCE((is_favorito), FALSE), 'tende_golear', CAST(NULL AS STRING)) AS premissa,
                's_gf_venue' AS insumo,
                CAST(s_gf_venue AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_favorito), FALSE), 'tende_golear', CAST(NULL AS STRING)) AS premissa,
                's_ga_venue' AS insumo,
                CAST(s_ga_venue AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_favorito), FALSE), 'adversario_fragil_fora', CAST(NULL AS STRING)) AS premissa,
                'o_ga_venue' AS insumo,
                CAST(o_ga_venue AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_favorito AND s_is_home), FALSE), 'mando_forte', CAST(NULL AS STRING)) AS premissa,
                'pct_pts_home' AS insumo,
                CAST(pct_pts_home AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_favorito AND competition IN ('brasileirao', 'serie_b', 'la_liga', 'premier_league', 'serie_a_ita', 'bundesliga', 'ligue_1', 'primeira_liga')), FALSE), 'sem_rodizio', CAST(NULL AS STRING)) AS premissa,
                's_rank' AS insumo,
                CAST(s_rank AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_favorito AND competition IN ('brasileirao', 'serie_b', 'la_liga', 'premier_league', 'serie_a_ita', 'bundesliga', 'ligue_1', 'primeira_liga')), FALSE), 'sem_rodizio', CAST(NULL AS STRING)) AS premissa,
                'n_teams' AS insumo,
                CAST(n_teams AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_azarao), FALSE), 'raramente_perde_por_2', CAST(NULL AS STRING)) AS premissa,
                's_n_games' AS insumo,
                CAST(s_n_games AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_azarao), FALSE), 'raramente_perde_por_2', CAST(NULL AS STRING)) AS premissa,
                's_lost2' AS insumo,
                CAST(s_lost2 AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_azarao), FALSE), 'defesa_fora_solida', CAST(NULL AS STRING)) AS premissa,
                's_ga_venue' AS insumo,
                CAST(s_ga_venue AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_azarao), FALSE), 'favorito_irregular', CAST(NULL AS STRING)) AS premissa,
                'o_n_games' AS insumo,
                CAST(o_n_games AS FLOAT64) AS valor
            ),
            STRUCT(
                IF(COALESCE((is_azarao), FALSE), 'favorito_irregular', CAST(NULL AS STRING)) AS premissa,
                'o_won2' AS insumo,
                CAST(o_won2 AS FLOAT64) AS valor
            )
        ])
